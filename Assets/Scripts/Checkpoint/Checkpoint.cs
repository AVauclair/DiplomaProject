...
public class Checkpoint : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            if (FindObjectOfType<PlayerController>().canSetCheckpoint == true)
            {
                FindObjectOfType<PlayerController>().canSetCheckpoint = false;
                StartCoroutine(DropCheckpointTime());
        ...
        IEnumerator DropCheckpointTime()
        {
            yield return new WaitForSeconds(2);
            FindObjectOfType<PlayerController>().canSetCheckpoint = true;
        }
    }
}
